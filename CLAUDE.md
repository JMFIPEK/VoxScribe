...
| `macos/` | Experimental macOS system-audio helper (Swift/ScreenCaptureKit) — see `macos/README.md` for build steps and known open issues |
...
- macOS (Apple Silicon) — experimental; microphone recording works, system-audio recording is experimental/untested (ScreenCaptureKit helper, see `macos/README.md`), no combined mic+system yet; transcription runs on CPU/MPS instead of CUDA
